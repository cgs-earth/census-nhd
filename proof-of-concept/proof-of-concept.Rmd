...
Consider the simplest variable available: Total Population (residing in a Census Block). If one were to divide a Census Block into two parts each with $1/2$ the area of Block, this assumption would imply that each part would have $1/2$ of the Total Population. In the figure below, compare the resulting estimated population densities for Blocks and Catchments (normalizing estimated population counts by the areas of each respective polygon)
...
cat_pop <- b %>% group_by(NHDPlusID) %>% summarise(pop = sum(P1_001N * prop_block_in_catchment))
blocks_pop <- b %>% group_by(GEOID20) %>% summarise(pop = mean(P1_001N))

cat_pop$popdense = 10^6 * cat_pop$pop/st_area(cat_pop)
blocks_pop$popdense =  10^6 * blocks_pop$pop/st_area(blocks_pop)
blocks_pop2 <- blocks_pop[which(as.numeric(blocks_pop$popdense) < 7350),]

map2 <- sync(mapview(blocks_pop2,
                     zcol = "popdense",
                     layer.name = "Pop. Density by Block") +
             mapview(cat_pop, 
             ...
             mapview(cat_pop,
                     zcol = "popdense",
                     layer.name = "Pop. Density Est. by Catchment")
             )
